...
public class DataTables implements java.io.Serializable {
	...
	private String timeMin;// 排序方式desc or asc
	private String timeMax;// 排序方式desc or asc
	...
	public DataTables(HttpServletRequest request, String subSQL){
		...
		timeMin = StringUtils.isEmpty(request.getParameter("timeMin"))?null:request.getParameter("timeMin");
		timeMax = StringUtils.isEmpty(request.getParameter("timeMax"))?null:request.getParameter("timeMax");
		order = request.getParameter("order[0][dir]");
		column = request.getParameter("columns["+request.getParameter("order[0][column]")+"][data]");
		subSQL = "limit "+start+","+length;
		this.setSubSQL(subSQL);
	}
