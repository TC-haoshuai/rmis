...
$(function($) {
	datatable = $('.table-sort').DataTable({
		...
		ajax: {
			url: contextPath + "/comment/getPageList",
			type: 'post',
			data: function(d) {
				d.search = $('#search').val();
			}
		...
		columnDefs: [{
			...
			render: function(data, type, row, meta) {
				var btns = new Array('review', 'del');
				return getDTOperateBtn(btns, row.id, 'comment-reply.jsp', row.status, '/comment/del/', reloadTable);
			}
		}],
	...
	switch(status) {
		case 0:
			layer.confirm('审核文章?', {
				...
				function() {
					changeStatus('/comment/update/', id, 2, reloadTable);
				},
				function() {
					changeStatus('/comment/update/', id, 1, reloadTable);
				});
			break;
		case 1:
			layer.confirm('重审文章?', {
		...
		case 2:
			layer.confirm('撤回文章?', {
				...
				function() {
					changeStatus('/comment/update/', id, 3, reloadTable);
				});
			break;
		case 3:
			layer.confirm('发布文章?', {
